...
// 💡 UBAH LAYOUT: Ganti ke layout instansi Anda jika berbeda
...
// === 1. DEFINISI TIPE DATA (Tambahan untuk Instansi) ===
interface Paket {
    id_paket: string;
    nama_paket: string;
    harga: number;
...
interface RiwayatItem {
    ...
    // 💡 TAMBAHAN: Bisa tambahkan info user yang beli (jika perlu)
    // user_pembeli_nama?: string; 
}
...
interface TransaksiPageProps {
    ...
    riwayat: RiwayatItem[];
    daftar_paket: Paket[];
    // 💡 TAMBAHAN: Data instansi, user, dll.
...
export default function TransaksiDanTokenInstansi({ 
    saldo_token = 0, 
    rincian_token = [], 
    riwayat = [], 
    daftar_paket = [], 
    instansi_nama = 'Instansi Anda', // Default untuk Instansi
    flash 
}: TransaksiPageProps) {
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            // 💡 UBAH ENDPOINT: Sesuaikan dengan endpoint checkout untuk Instansi
            const response = await axios.post('/instansi/transaksi/checkout', {
                paket_id: selectedPaket.id_paket,
                quantity: quantity
                // 💡 TAMBAHAN: Bisa kirim data instansi jika belum otomatis di backend
                // instansi_id: instansi_id_props, 
            });
            ...
            if (result.snap_token) {
                ...
                window.snap.pay(result.snap_token, {
                    onSuccess: function(result: any) {
                        alert("Pembayaran Berhasil! Saldo Token Instansi akan segera bertambah.");
                    ...
                    onClose: function() {
                        // 💡 OPSIONAL: Bisa panggil API ke backend untuk membatalkan/mencatat close
                        // axios.post('/instansi/transaksi/cancel-midtrans', { order_id: result.order_id });
    ...
    return (
        // 💡 DASHBOARD LAYOUT: Pastikan menggunakan layout yang sesuai untuk instansi
        <InstansiLayout>
            <Head title={`Dompet & Transaksi - ${instansi_nama}`} /> 
            ...
            <div className="p-6 min-h-screen bg-[#FAFAFA] font-sans text-[#2A2A2A]">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
                    <div>
                        {/* 💡 UBAH JUDUL: Menambahkan konteks instansi */}
                        <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Dompet & Transaksi <span className='text-yellow-600'>({instansi_nama})</span></h1>
                        <p className="text-gray-500 mt-1">Kelola token dan pantau riwayat pembelian untuk instansi Anda.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-8 space-y-8">
                        ...
                        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-yellow-400 via-yellow-500 to-yellow-600 p-8 text-white shadow-xl">
                            ...
                            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                                <div>
                                    <p className="text-yellow-100 font-medium text-sm uppercase tracking-wider mb-1">Total Saldo Aktif Instansi</p>
                                    ...
                                    <p className="mt-2 text-sm text-white opacity-90">Token ini dapat digunakan oleh semua anggota instansi Anda.</p>
                                </div>
                        ...
                        <div>
                            ...
                            {rincian_token.length > 0 ? (
                                ...
                                <div className="bg-white rounded-2xl p-8 border-2 border-dashed border-gray-200 text-center">
                                    ...
                                    <p className="text-gray-500 font-medium">Belum ada paket token yang tersedia untuk instansi.</p>
                    ...
                    {/* === RIGHT COLUMN (HISTORY - TETAP SAMA) === */}
                            ...
                            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                                {riwayat.length === 0 ? (
                                    <div className="h-full flex flex-col items-center justify-center text-gray-400 text-sm py-10">
                                        ...
                                        <p>Belum ada riwayat pembelian token instansi.</p>
                                    </div>
                                ) : (
                                        ...
                                        {riwayat.map((trx, index) => (
                                            <div key={index} className="relative pl-6 border-l-2 border-gray-100 last:border-0 pb-1">
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    {/* 💡 OPSIONAL: Tampilkan nama user yang melakukan pembelian */}
                                                    {/* {trx.user_pembeli_nama && <p className="text-[10px] text-gray-400 mt-0.5">Oleh: {trx.user_pembeli_nama}</p>} */}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                            ...
                            <div className="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-5xl border border-gray-200">
                                ...
                                <div className="bg-white px-6 py-5 border-b border-gray-100 flex justify-between items-center sticky top-0 z-20">
                                    <div>
                                        <h3 className="text-xl font-bold text-gray-900">Beli Token Baru untuk Instansi</h3>
                                        <p className="text-sm text-gray-500">Pilih paket yang sesuai dengan kebutuhan {instansi_nama}.</p>
                                    </div>
                                ...
                                <div className="flex flex-col lg:flex-row h-full max-h-[80vh] lg:max-h-[70vh]">
                                    ...
                                    {/* KOLOM KIRI: Pilihan Paket (TETAP SAMA) */}
                                    ...
                                    {/* KOLOM KANAN: Checkout Summary (TETAP SAMA) */}
