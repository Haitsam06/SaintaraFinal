...
// Tidak perlu HiMicrophone/HiUserGroup/HiVideoCamera lagi karena list agenda list dihapus

// --- Tipe Data (Disesuaikan untuk menerima props) ---
type CalendarEvent = {
    id: number;
...
interface CalendarPageProps {
    events: CalendarEvent[]; // Data untuk kalender grid
    upcoming: any[]; // Data ini diabaikan (karena list dihapus)
    currentDateStr: string; // Tanggal awal yang dilihat (misal: 2025-11-22)
}
...
export default function Calendar({ events, upcoming, currentDateStr }: CalendarPageProps) {
    ...
    // === LOGIKA NAVIGASI (Menggunakan Inertia router) ===
    ...
    const goToToday = () => {
        router.get('/kalender-agenda', {}, { preserveScroll: true });
    };

    // --- LOGIKA BUILD GRID (Menggunakan useMemo untuk performa) ---
                        ...
                        <div className="w-full">
                            ...
                            <div className="grid grid-cols-7 grid-rows-6 gap-2">
                                {calendarDays.map(({ day, isCurrentMonth, dateStr, isToday, events }) => {
                                    ...
                                    return (
                                        <div key={dateStr} className={`h-32 border p-2 ${highlightClass} flex flex-col items-start overflow-hidden rounded-lg transition-colors ${isCurrentMonth ? 'bg-white hover:bg-gray-50' : 'bg-gray-50'}`}>
                                            {/* NOMOR TANGGAL */}
