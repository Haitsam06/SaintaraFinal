...
export default function Pemasukan({ transaksi, totalPemasukan, countBulanIni }: Props) {
    ...
    const openAddModal = () => {
        setIsEditMode(false);
        reset();
        setData('tipe', 'pemasukan'); // Pastikan tipe diset
        setIsModalOpen(true);
    };
            ...
            <div className="animate-fade-in space-y-6 font-poppins">
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div className="rounded-2xl border border-green-200 bg-green-50 p-6">
                        ...
                        <h3 className="mt-1 text-3xl font-bold text-green-600">{formatRupiah(totalPemasukan)}</h3>
                    </div>
                    <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6">
                        <p className="text-sm font-medium text-blue-600">Transaksi Bulan ini:</p>
                        <h3 className="mt-1 text-3xl font-bold text-blue-600">{countBulanIni}</h3>
                    </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-between gap-4 rounded-2xl bg-gray-50 p-4 md:flex-row">
                    <div className="relative w-full md:w-1/2">
                        <input type="text" placeholder="Cari transaksi..." className="text-slate-800 w-full rounded-full border-none bg-gray-100 py-2 pr-4 pl-10 focus:ring-2 focus:ring-yellow-400" />
                    ...
                    <button onClick={openAddModal} className="flex items-center gap-2 rounded-full bg-yellow-400 px-6 py-2 font-bold text-gray-900 shadow-md transition-all hover:scale-105 hover:bg-yellow-500">
                        <HiPlus className="h-5 w-5" /> Tambah Pemasukan
                    </button>
                </div>
                            ...
                            {transaksi.data.length > 0 ? (
                                transaksi.data.map((item, idx) => (
                                    <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-5 text-green-600">{formatRupiah(item.jumlah)}</td>
                ...
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={isEditMode ? 'Edit Pemasukan' : 'Tambah Pemasukan'}>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <input type="number" value={data.jumlah} onChange={(e) => setData('jumlah', e.target.value)} className={inputClass} placeholder="Contoh: 150000" required />
