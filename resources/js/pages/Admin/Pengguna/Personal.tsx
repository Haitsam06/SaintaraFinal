import DashboardLayout from '@/layouts/dashboardLayoutAdmin'; // <-- PERBAIKAN: Menggunakan nama export yang benar
...
// --- KOMPONEN MODAL DENGAN PORTAL ---
const Modal = ({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children: ReactNode }) => {
    if (!isOpen) return null;

    // Cari elemen target Portal yang kita buat di app.blade.php
    const modalRoot = document.getElementById('modal-root');

    // Pastikan modalRoot ada sebelum me-render
    if (!modalRoot) return null;

    // Gunakan createPortal untuk merender modal di luar DOM utama
    return ReactDOM.createPortal(
        // Wrapper: z-[9999] dan fixed inset-0 sekarang dijamin bekerja di jendela
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-md transition-all">
            {/* Konten Modal: Lebar dan tinggi maksimum */}
        ...
        modalRoot // Target DOM tempat modal akan dipindahkan
    )};
