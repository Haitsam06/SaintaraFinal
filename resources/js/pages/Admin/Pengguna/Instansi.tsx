import DashboardLayout from '@/layouts/dashboardLayoutAdmin'; // <-- PERBAIKAN: Menggunakan nama export yang benar (DashboardLayout)
import { Head, Link, router, useForm, usePage } from '@inertiajs/react';
import { FormEvent, ReactNode, useState } from 'react';
import { HiPencil, HiPlus, HiSearch, HiTrash, HiX } from 'react-icons/hi';
import ReactDOM from 'react-dom'; // <-- TAMBAHKAN: Diperlukan untuk React Portal
...
// --- KOMPONEN MODAL (MENGGUNAKAN REACT PORTAL) ---
const Modal = ({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children: ReactNode }) => {
    if (!isOpen) return null;

    // Ambil elemen target dari app.blade.php
    const modalRoot = document.getElementById('modal-root');

    if (!modalRoot) return null; // Fallback jika root tidak ditemukan

    return ReactDOM.createPortal(
        // Wrapper: z-[9999] memastikan modal selalu di atas semua elemen layout (sidebar, header)
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-md transition-all">
            {/* Konten Modal: Lebar 3xl, tinggi maksimum 90% dari viewport, dan bisa di-scroll */}
            <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto scale-100 transform rounded-2xl bg-white p-8 shadow-2xl transition-all">
                
                {/* Header Modal: Sticky untuk scrolling konten */}
                <div className="mb-6 flex items-center justify-between border-b border-gray-100 pb-4 sticky top-0 bg-white z-10">
        ...
        modalRoot // Target DOM tempat modal akan dirender
    );
};
...
export default function InstansiPage({ users, filters }: Props) {
    ...
    return (
        <DashboardLayout> {/* <-- PENGGUNAAN NAMA LAYOUT YANG BENAR */}
