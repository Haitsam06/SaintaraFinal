...
interface PageProps {
    ...
    testimonials: Testimonial[]; // Data dari Controller masuk ke sini
    [key: string]: any;
}

export default function Landing() {
    // 1. MENGAMBIL PROPS DARI CONTROLLER
    // 'testimonials' dikirim dari LandingController -> index()
    const { auth, testimonials } = usePage<PageProps>().props;
    const user = auth?.user;

    // Pastikan reviews selalu berupa array (Safety check)
    const reviews = Array.isArray(testimonials) ? testimonials : [];

    // Logic Pengecekan Role (Optional jika ingin dipakai di UI)
    const isInstansi = user?.role_id === 4;
    const isPersonal = user?.role_id === 3;

    // 2. LOGIC URL CTA (TOMBOL UTAMA)
    const getTargetUrl = () => {
        if (!user) {
            return '/login';
        }
        const rid = Number(user.role_id);
        if (rid === 1 || rid === 2) return '/admin/dashboardAdmin';
        if (rid === 4) return '/instansi/dashboardInstansi';
        return '/personal/dashboardPersonal';
    };
    ...
    // 5. DATA STATIS (KARAKTER & FITUR)
    const characterTypes = [
        { name: 'Pemikir Introvert', desc: 'Analis yang mendalam, logis, dan lebih suka bekerja sendiri.', imageSrc: '/assets/9karakter/PemikirIntrovert.png' },
        { name: 'Pemikir Extrovert', desc: 'Pemimpin tegas, strategis, dan suka mengatur sistem.', imageSrc: '/assets/9karakter/PemikirExtrovert.png' },
        { name: 'Pengamat Introvert', desc: 'Praktis, teliti, dan mengandalkan fakta nyata.', imageSrc: '/assets/9karakter/PengamatIntrovert.png' },
        { name: 'Pengamat Extrovert', desc: 'Energik, spontan, dan suka menikmati momen saat ini.', imageSrc: '/assets/9karakter/PengamatExtrovert.png' },
        { name: 'Perasa Introvert', desc: 'Penuh empati, peduli, dan setia pada nilai-nilai pribadi.', imageSrc: '/assets/9karakter/PerasaIntrovert.png' },
        { name: 'Perasa Extrovert', desc: 'Karismatik, inspiratif, dan mudah bergaul dengan orang lain.', imageSrc: '/assets/9karakter/PerasaExtrovert.png' },
        { name: 'Pemimpi Introvert', desc: 'Idealis, kreatif, dan mencari makna mendalam dalam hidup.', imageSrc: '/assets/9karakter/PemimpiIntrovert.png' },
        { name: 'Pemimpi Extrovert', desc: 'Inovatif, antusias, dan pandai menghubungkan ide-ide.', imageSrc: '/assets/9karakter/PemimpiExtrovert.png' },
        { name: 'Penggerak', desc: 'Adaptif, pemecah masalah, dan berorientasi pada tindakan.', imageSrc: '/assets/9karakter/Penggerak.png' },
    ];
    ...
    return (
        <>
            ...
            {/* Testimoni (DYNAMIC DARI DATABASE) */}
            {/* Testimoni (DYNAMIC DARI DATABASE) */}
            <section id="testimoni" className="bg-[#FFF3D8] py-20">
                <div className="mx-auto max-w-screen-lg px-4">
                    ...
                    {/* CONTAINER: Tetap 'space-y-6' agar desain menumpuk ke bawah (SAMA PERSIS SEPERTI ASLINYA) */}
                    <div className="space-y-6">
                        {reviews.length > 0 ? (
                            reviews.map((testimonial, index) => (
                                <div
                                    key={testimonial.id || index}
                                    // STYLE CARD: Tetap flex-row (foto kiri, teks kanan)
                                ...
                                >
                                    <div className="flex-shrink-0">
                                        <img
                                            ...
                                            onError={(e) => {
                                                (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${testimonial.name}&background=FCD34D&color=fff`;
                                            }}
                                        />
                                    </div>
                                    <div>
                                        <h3 className="font-poppins text-xl font-bold text-saintara-black">{testimonial.name}</h3>
                                        <p className="mt-1 text-gray-600 italic">"{testimonial.text}"</p>
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="py-10 text-center" data-aos="fade-up">
                                <p className="text-lg text-gray-500">Belum ada testimoni saat ini. Jadilah yang pertama mencoba!</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {/* Pricing Cards */}
            <section id="harga" className="min-h-screen bg-gray-50 px-4 py-12 font-sans sm:px-6 lg:px-8">
            ...
            {/* CTA Penutup */}
            <section id="cta-penutup" className="cta-gradient bg-saintara-black py-20">
                <div className="mx-auto max-w-screen-md px-4 text-center">
                    ...
                    <Link href={ctaLink} className="mt-8 inline-block transform rounded-lg bg-saintara-yellow px-12 py-4 text-lg font-bold text-saintara-black transition-all duration-300 hover:scale-105 hover:bg-white hover:text-saintara-black focus:ring-4 focus:ring-yellow-300" data-aos="zoom-in" data-aos-delay="200">
