...
export default function Welcome() {
    // 1. Mengambil Props Auth dari Inertia (Global State)
    // Pastikan di Middleware HandleInertiaRequests Laravel Anda sudah share 'auth.user'
    const { auth } = usePage().props as any;
    const user = auth?.user;

    // LOGIC TAMBAHAN UNTUK PENGECEKAN ROLE
    // Pastikan role di database Anda stringnya 'personal' dan 'instansi'
    const isInstansi = user?.role === 'instansi';
    const isPersonal = user?.role === 'personal';

    // URL Tujuan
    const urlPersonal = user ? '/personal/dashboardPersonal' : '/login';
    ...
    // 2. Logic Menentukan Tujuan Link
    // Jika belum login -> ke /login
    // Jika sudah login -> cek role dan arahkan ke dashboard yang sesuai (berdasarkan web.php)
    const getTargetUrl = () => {
        if (!user) {
            return '/login';
        }
        ...
        // Sesuaikan 'admin', 'instansi', 'personal' dengan value di database Anda.
        if (user.role === 'admin') {
            return '/admin/dashboardAdmin';
        } else if (user.role === 'instansi') {
            return '/instansi/dashboardInstansi';
        } else {
            // Default ke Personal jika role lain atau user biasa
            return '/personal/daftarTesPersonal';
        }
    };

    // Simpan URL tujuan ke dalam variabel
        ...
        <>
                ...
                <div className="relative mx-auto grid max-w-screen-xl items-center gap-12 px-4 py-20 lg:grid-cols-2 lg:py-28">
                    <div className="text-center lg:text-left" data-aos="fade-right">
                        ...
                        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row lg:justify-start">
                            {/* TOMBOL UTAMA: Menggunakan variable ctaLink */}
                            <Link
                                href={ctaLink}
                ...
                <div className="mx-auto grid max-w-screen-xl items-center gap-12 px-4 md:grid-cols-2">
                    <div data-aos="fade-right">
                        ...
                        <p className="mt-6 leading-relaxed text-gray-600">
                            Saintara adalah sebuah platform inovatif yang dirancang untuk membantu Anda memahami cetak biru alami kepribadian Anda. Kami percaya bahwa setiap orang memiliki potensi unik yang, jika dipahami dan dikembangkan, dapat membawa pada kehidupan yang lebih memuaskan dan
                            sukses.
                        </p>
                        <div className="mt-6">
                        ...
                        {characterTypes.map((char, index) => (
                            <div key={index} className="group relative h-80 overflow-hidden rounded-xl shadow-lg" data-aos="zoom-in" data-aos-delay={index * 100}>
                                ...
                                <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-saintara-black/90 p-6 text-center text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                                    ...
                                    <Link href="/dashboard/articles" className="rounded-full border-2 border-white px-6 py-2 transition-colors hover:bg-white hover:text-saintara-black" aria-label={`Learn more about ${char.name}`}>
                ...
                <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 md:grid-cols-3">
                    ...
                    <div id="harga" className="relative flex flex-col rounded-[2rem] bg-white p-8 shadow-xl transition-transform hover:-translate-y-1">
                        ...
                        <div className="mt-10 space-y-3">
                            ...
                            {/* LOGIC TOMBOL PERSONAL */}
                            {isInstansi ? (
                                // JIKA LOGIN SEBAGAI INSTANSI -> TOMBOL MATI
                                ...
                                // JIKA LOGIN PERSONAL / BELUM LOGIN -> TOMBOL AKTIF
                    ...
                    <div className="relative flex flex-col rounded-[2rem] bg-white p-8 shadow-xl transition-transform hover:-translate-y-1">
                        ...
                        <div className="mt-10 space-y-3">
                            ...
                            {/* LOGIC TOMBOL INSTANSI */}
                            {isPersonal ? (
                                // JIKA LOGIN SEBAGAI PERSONAL -> TOMBOL MATI
                                <button disabled className="w-full cursor-not-allowed rounded-xl border-2 border-gray-200 bg-gray-100 py-3 text-sm font-bold text-gray-400">
                                    Khusus Akun Instansi
                                </button>
                            ) : (
                                // JIKA LOGIN INSTANSI / BELUM LOGIN -> TOMBOL AKTIF
                    ...
                    <div className="relative flex flex-col rounded-[2rem] border-none bg-[#FFF9E5] p-8 shadow-xl transition-transform hover:-translate-y-1">
                        ...
                        <div className="mt-10 space-y-3">
                            ...
                            {/* LOGIC TOMBOL GIFT (Sama dengan Personal) */}
                            {isInstansi ? (
                                // JIKA LOGIN SEBAGAI INSTANSI -> TOMBOL MATI
            ...
            <section id="cta-penutup" className="cta-gradient py-20">
                <div className="mx-auto max-w-screen-md px-4 text-center">
                    ...
                    {/* CTA LINK JUGA DIPERBARUI DI SINI */}
                    <Link
                        href={ctaLink}
