...
import { HiBell, HiCalendar, HiCog, HiCurrencyDollar, HiHome, HiLogout, HiQuestionMarkCircle, HiStar, HiUserGroup, HiUsers } from 'react-icons/hi';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const menuItems = [
        ...
        { name: 'Review', href: '/admin/reviews', icon: HiStar }, // Ikon Token bisa disesuaikan
            ...
            <aside className={`z-30 flex flex-shrink-0 flex-col bg-[#0F0F0F] text-white transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-72' : 'w-20'} `}>
                ...
                <nav className="custom-scrollbar flex-1 space-y-1.5 overflow-y-auto px-4 py-6">
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link key={item.name} href={item.href} className={`group relative flex items-center rounded-xl px-4 py-3.5 transition-all duration-200 ${isActive ? 'scale-[1.02] transform bg-yellow-400 font-bold text-black shadow-lg shadow-yellow-400/20' : 'text-gray-400 hover:bg-gray-800 hover:text-white'} `}>
                ...
                <header className="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-gray-200 bg-white/80 px-8 backdrop-blur-md">
                    ...
                    <div className="flex items-center gap-4">
                        <Link href="/admin/profileAdmin" className="group flex items-center gap-3 rounded-full border border-gray-100 bg-white py-1.5 pr-4 pl-2 shadow-sm transition-all hover:border-yellow-300 hover:shadow-md">
                            <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full border border-yellow-200 bg-yellow-100">{user.foto ? <img src={`/storage/${user.foto}`} alt="Avatar" className="h-full w-full object-cover" /> : <span className="text-sm font-bold text-yellow-700">{displayName.charAt(0).toUpperCase()}</span>}</div>
