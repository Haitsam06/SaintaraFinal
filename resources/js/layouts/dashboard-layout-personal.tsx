...
import { HiBell, HiCog, HiCreditCard, HiDocumentText, HiGift, HiHome, HiLogout, HiSupport, HiUserCircle } from 'react-icons/hi';

// --- DAFTAR MENU (DISESUAIKAN DENGAN WEB.PHP) ---
const menuItems = [
    { name: 'Beranda', href: '/personal/dashboard', icon: HiHome },
    { name: 'Profil Saya', href: '/personal/profile', icon: HiUserCircle },
    { name: 'Daftar Tes', href: '/personal/daftar-tes', icon: HiDocumentText },
    { name: 'Dompet & Token', href: '/personal/transaksi-token', icon: HiCreditCard },
    { name: 'Hadiah & Donasi', href: '/personal/donasi', icon: HiGift },
    { name: 'Pusat Bantuan', href: '/personal/bantuan', icon: HiSupport },
    { name: 'Pengaturan', href: '/personal/pengaturan', icon: HiCog },
];
                    ...
                    <Link href="/" className="group flex items-center gap-3">
                        ...
                        {isSidebarOpen && (
                            <div className="flex flex-col">
                                ...
                                <span className="text-[10px] tracking-widest text-gray-400 uppercase">Personal Area</span>
                            </div>
                        )}
                    </Link>
                </div>
            ...
            <div className="relative flex flex-1 flex-col overflow-hidden">
                ...
                <header className="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-gray-200 bg-white/80 px-8 backdrop-blur-md">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Link href="/personal/profile" className="group flex items-center gap-3 rounded-full border border-gray-100 bg-white py-1.5 pr-4 pl-2 shadow-sm transition-all hover:border-yellow-300 hover:shadow-md">
                            <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full border border-yellow-200 bg-yellow-100">
                                {user.foto ? <img src={user.foto} alt="Avatar" className="h-full w-full object-cover" /> : <span className="text-sm font-bold text-yellow-700">{displayName.charAt(0)}</span>}
                            </div>
